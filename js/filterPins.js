...
(function () {
  window.filterPins = function () {
    ...
    var filteredPins = [];
    for (var i = 0; filteredPins.length < window.util.MAX_OFFERS && dataCards.length - 1 > i; i++) {
      var arrayOfSimilarFeatures = housingFeaturesInputs.filter(function (item) {
        return filterFeatures(dataCards[i], item);
      });
      if (
        (dataCards[i].offer.type === housingTypeSelect.value || housingTypeSelect.value === window.util.ALL_CHOISE)
          && (dataCards[i].offer.rooms === Number(housingRooms.value) || housingRooms.value === window.util.ALL_CHOISE)
          && (dataCards[i].offer.guests === Number(housingGuests.value) || housingGuests.value === window.util.ALL_CHOISE)
